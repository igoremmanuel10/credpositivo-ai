...
usersRouter.post('/api/register', async (req, res) => {
  try {
    const { nome, cpf, email, telefone, senha, referred_by_code } = req.body;
    ...
    const { rows } = await db.query(
      `INSERT INTO users (nome, cpf, email, telefone, senha, referred_by_code)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, nome, cpf, email, telefone, tipo, status, created_at`,
      [nome, cpf, email, telefone || null, hashedPassword, referred_by_code || null]
    );
...
// PUT /api/admin/users/:id/password — Admin changes client password
usersRouter.put('/api/admin/users/:id/password', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error('[API] Change user password error:', err.message);
...
usersRouter.put('/api/admin/change-password', async (req, res) => {
  try {
    ...
    const { rows } = await db.query('SELECT senha FROM managers WHERE id = $1', [adminId]);
    if (rows.length === 0) {
      return res.status(404).json({ success: false, error: 'Admin nao encontrado' });
    }

    const passwordMatch = await bcrypt.compare(senha_atual, rows[0].senha);
    if (!passwordMatch) {
      return res.status(401).json({ success: false, error: 'Senha atual incorreta' });
    }

    const hashedPassword = await bcrypt.hash(senha_nova, 10);
    await db.query('UPDATE managers SET senha = $1 WHERE id = $2', [hashedPassword, adminId]);
...
// PUT /api/users/me/password — Client changes their own password
usersRouter.put('/api/users/me/password', async (req, res) => {
  try {
    const { user_id, senha_atual, senha_nova } = req.body;

    if (!user_id || !senha_atual || !senha_nova) {
      return res.status(400).json({ success: false, error: 'user_id, senha_atual e senha_nova sao obrigatorios' });
    }

    if (senha_nova.length < 6) {
      return res.status(400).json({ success: false, error: 'Nova senha deve ter no minimo 6 caracteres' });
    }

    const { rows } = await db.query('SELECT senha FROM users WHERE id = $1 AND ativo = true', [user_id]);
    if (rows.length === 0) {
      return res.status(404).json({ success: false, error: 'Usuario nao encontrado' });
    }
    ...
    const hashedPassword = await bcrypt.hash(senha_nova, 10);
    await db.query('UPDATE users SET senha = $1 WHERE id = $2', [hashedPassword, user_id]);

    res.json({ success: true, message: 'Senha alterada com sucesso' });
  } catch (err) {
    console.error('[API] User change-password error:', err.message);
    res.status(500).json({ success: false, error: 'Erro ao alterar senha' });
  }
});
