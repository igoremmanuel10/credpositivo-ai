...
function buildWhatsAppMessage(eventType, leadName, produto, webCallUrl) {
  ...
  switch (eventType) {
    case 'purchase_abandoned': {
      ...
      return [
        `Oi${nome ? ' ' + nome : ''}! Aqui e o Augusto da CredPositivo.`,
        '',
...
function buildCallOverrides(eventType, leadName, produto, eventData) {
  ...
  switch (eventType) {
    case 'purchase_abandoned': {
      overrides.firstMessage = `Oi${leadName !== 'amigo' ? ', ' + leadName : ''}! Aqui e o Augusto da CredPositivo. Que bom que voce entrou na chamada! Vi que voce estava dando uma olhada no nosso servico${produto ? ' de ' + formatProductName(produto) : ''}. Posso te ajudar com alguma duvida?`;
    ...
    case 'diagnosis_completed': {
      overrides.firstMessage = `Oi${leadName !== 'amigo' ? ', ' + leadName : ''}! Aqui e o Augusto da CredPositivo. Que bom que voce entrou na chamada! Seu diagnostico de credito ficou pronto e tem algumas coisas importantes que quero te explicar. Pode falar agora?`;
